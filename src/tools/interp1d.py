'''
Script for 1D linear interpolation on GPU using PyTorch.
    ...
    def forward(ctx, x, y, xnew, out=None):
        ...
        if out is not None:
            if out.numel() != shape_ynew[0]*shape_ynew[1]:
                ...
                ynew = out.reshape(shape_ynew).long()
        if out is None:
            ynew = torch.zeros(*shape_ynew, device=device).long()
        ...
        ind = torch.clamp(ind, 0, v['x'].shape[1] - 2)
