...
def test_roundtrip_2d(pdb: str = "3uat.pdb", angles: tuple[float, float, float] = (135, 45, 120)):
    coordinates = np.stack([atom.coordinates for atom in AtomLine.from_pdb(DATA / pdb)])

    xmipp = Xmipp(DATA / "test_data")
    mrcs_path, angles_path = xmipp.simulate(DATA / pdb, 5, 10)
    pixels_stack = mrcfile(mrcs_path)
    angles = starfile.read(angles_path)
    for pixels,angles in zip(pixels_stack,angles.itertuples()):
        registrator = Registrator(n_spherical=36, n_inplane=36, l_max=5, k_res=2).load_2d(pixels, thresh=0.8)
        prediction = tuple(registrator.align(coordinates, sigma=1.0).values())
